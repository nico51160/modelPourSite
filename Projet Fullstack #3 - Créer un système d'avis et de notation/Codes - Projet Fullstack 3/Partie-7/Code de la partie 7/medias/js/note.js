...
let notemoyenne  = document.querySelector('#noteMoyenne span');
/****** Liste des variables */
/***************************/


/****** Script étoiles */
for(let a = 0; a < etoiles.length; a+=1) {
    etoiles[a].addEventListener('mouseover', function() {
        this.classList.add('dessus');
        afficherNote.classList.add('active');
        afficherNote.innerText = a+1;
    });
    etoiles[a].addEventListener('mouseout', function() {
        this.classList.remove('dessus');
        afficherNote.classList.remove('active');
    });
    etoiles[a].addEventListener('click', function(){
        erreur.innerText = '';
...
let FonctionModale = (rep2) => {
    ...
    setTimeout( function() {
        ...
        reponse.innerText = rep2.phrase;
        notemoyenne.innerText = rep2.noteavg;
    }, 1000);
}
